...
(() => {
  ...
  // =============================================================================
  // Self-Healing Module Integration
  // =============================================================================
  // Note: Modules are loaded via manifest.json content_scripts before content.js
  // Global references for self-healing engine integration
  ...
  // Setup auto-save/restore if persistence module is available
  if (formPersistence) {
  ...
  const MAX_SHADOW_HTML_CHARS = 50000;
  const MAX_SHADOW_ROOTS = 80;
  const MAX_EXECUTE_JS_CODE_CHARS = 20000;
  const MAX_EXECUTE_JS_RESULT_CHARS = 50000;
  const DEFAULT_EXECUTE_JS_TIMEOUT_MS = 5000;
  const DOM_SKILL_MEMORY_KEY = '__bilge_dom_skill_memory_v1';
  const DOM_SKILL_MEMORY_MAX = 300;
  const DOM_SKILL_MEMORY_TTL_MS = 45 * 24 * 60 * 60 * 1000;
  const NATURAL_COMMAND_MEMORY_KEY = '__bilge_natural_command_memory_v1';
  const NATURAL_COMMAND_MEMORY_MAX = 300;
  const NATURAL_COMMAND_MEMORY_TTL_MS = 60 * 24 * 60 * 60 * 1000;
  ...
  function setNativeValue(element, value) {
    ...
    // Cross-realm safe: if the node comes from an iframe, use that iframe's prototypes.
  ...
  // =============================================================================
  // Heuristic Element Matching (ported from CaravanFlow)
  // =============================================================================
  ...
  async function writeDomSkillMemory(entries) {
    ...
    try {
      ...
      // Non-fatal. Runtime still works even if persistence fails.
    }
  }
  ...
  async function matchDomSkill(intent, target) {
    ...
    const ranked = memory
      .map((entry) => ({ entry, score: scoreSkillEntry(entry, scope, intent, targetNorm) }))
  ...
  function normalizeNaturalCommandKey(value) {
    return String(value || '')
      .toLowerCase()
      .replace(/\bconfirmaton\b/g, 'confirmation')
      .replace(/\bconfirmtion\b/g, 'confirmation')
      .replace(/\bconfrimation\b/g, 'confirmation')
      .replace(/\bconfimation\b/g, 'confirmation')
  ...
  function pruneNaturalCommandMemory(entries) {
    ...
    return Array.from(byKey.values())
      ...
      .slice(0, NATURAL_COMMAND_MEMORY_MAX);
  }
  ...
  async function writeNaturalCommandMemory(entries) {
    ...
    try {
      ...
      // Non-fatal; command execution still succeeds even if memory persistence fails.
    }
  }
  ...
  async function resolveNaturalCommand(command) {
    ...
    // 3) Heuristic self-heal rewrites
  ...
  async function runNaturalCommandAutoDebug(command) {
    ...
    // Auto-debug script mode for "fill X from Y" / "copy Y into X" patterns.
    let target = '';
    let source = '';
  ...
  /**
   * Find element using heuristic matching when selector fails
   * @param {Object} request - Request with field, name, label, placeholder hints
   * @param {string} selector - Original selector for hint extraction
  ...
  // =============================================================================
  // Global Exports for Self-Healing Engine Integration
  // =============================================================================
  // Expose key functions for self-healing strategies to use
  ...
  function jsonSafe(value, seen = new WeakSet(), depth = 0) {
    ...
    if (t === 'string') return truncate(value, 8000);
    ...
    if (value instanceof Error) {
      return {
        ...
        stack: truncate(String(value.stack || ''), 4000),
      };
    }
  ...
  async function executeUserJavaScript(code, timeoutMs) {
    ...
    const timeout = Math.max(0, Math.min(Number(timeoutMs) || DEFAULT_EXECUTE_JS_TIMEOUT_MS, 30000));
  ...
  /**
   * Execute a natural language action parsed by cortex
   */
  ...
  async function executeClickAction(action, parsed) {
    // If using deictic reference, try to find element under cursor or use last focused
    ...
    // Try to find element by label/name/field
  ...
  async function executeTypeAction(action, parsed) {
    ...
    // If a field target is provided, resolve that specific input deterministically.
    let element = null;
    if (target) {
      element = await resolveFieldElement(target);
    } else {
      // Legacy fallback path when no target was specified.
  ...
  async function executeNavigateAction(action) {
    // Navigation actions are actually scripts
    if (action.code) {
      const AsyncFunction = Object.getPrototypeOf(async function () {}).constructor;
  ...
  // ═══════════════════════════════════════════════════════════════
  // RESILIENT NATURAL COMMAND EXECUTOR
  // ═══════════════════════════════════════════════════════════════

  /**
   * Execute natural language command with multi-level fallback
   */
  async function executeResilientNaturalCommand(command) {
    ...
    // LEVEL 1: Try Cortex (intelligent routing)
    if (window.BilgeCortex?.parseCommand || window.__bilge_cortex_loaded) {
      try {
        ...
        if (resolved && !resolved.error && resolved.executable) {
          ...
          if (execResult) {
            // Persist "repaired" commands so future invocations can skip the recovery loop.
            // We only remember self-healed commands to avoid storing every raw prompt (and their values).
    ...
    // LEVEL 2: Try Intelligent DOM Engine (no AI needed)
    ...
    // LEVEL 3: Pattern-based direct execution (last resort)
    try {
  ...
  /**
   * Direct pattern execution - no AI, no complex parsing
   */
  async function executeDirectPattern(command) {
    const raw = String(command || '');
    const cmd = raw.toLowerCase().trim();

    // Persist simple state per page so "continue/next" can advance field-by-field flows.
    ...
    const wantsNextField =
      /^(?:continue|next|proceed)(?:\s+(?:field|input))?\s*$/i.test(cmd) || /\bnext\s+field\b/i.test(raw);
    if (state.stepMode && wantsNextField) {
      return await fillVisibleFormFields({ step: true });
    }

    // ─── FILL FORM PATTERN (Conversational) ───
    const looksLikeFillForm =
      cmd.match(/^(fill|complete|populate|yes|do\s+it|please|continue|proceed)\s*(form|fields?|all|them|in)?(\s+now)?$/i) ||
      cmd.match(/^(yes\s+)?fill\s+them\s+in$/i) ||
      cmd.match(/^go\s+ahead$/i) ||
      (/\b(fill|complete|populate)\b/i.test(raw) && /\b(form|fields?)\b/i.test(raw));

    if (looksLikeFillForm) {
      // Allow an explicit "fill all" command to break out of step mode.
      const wantsAll =
        /\b(all|everything)\b/i.test(raw) && /\b(fields?|form)\b/i.test(raw) && !stepRequested;
    ...
    try {
      ...
      if (request.type === 'GET_PAGE_INFO') {
        ...
        sendResponse({
          ...
          html: truncate(bodyText, MAX_PAGE_TEXT_CHARS) // Text-only summary for context
        });
        return true;
      }
      ...
      if (request.type === 'CLICK_ELEMENT') {
        ...
        if (selector) {
          try {
            ...
            // Invalid selector, but might have heuristic hints
        ...
        // Fallback to heuristic matching if selector failed
      ...
      if (request.type === 'EXTRACT_FORM_FIELDS') {
        ...
        try {
          ...
          for (const element of allInputs) {
            ...
            // Skip hidden, submit, button, image, file, reset types
            ...
            // Build a unique selector for this field
            let selector = '';
            if (id) {
              ...
              // Fallback to nth-of-type selector
      ...
      if (request.type === 'EXECUTE_JS') {
        ...
        Promise.resolve()
          .then(() => executeUserJavaScript(code, timeoutMs))
          .then((result) => {
      ...
      if (request.type === 'CANCEL_CURRENT_ACTION') {
        ...
        // Currently we don't have a way to stop a running action easily,
        // but we can at least clear highlights.
        if (window.__bilge_runtime) {
          window.__bilge_runtime.updateHighlight(null);
        }
        sendResponse({ ok: true });
      ...
      // Handle natural language commands via cortex with resilient fallback
      if (request.type === 'EXECUTE_NATURAL_COMMAND') {
        ...
        Promise.resolve()
          .then(() => executeResilientNaturalCommand(command))
          .then((result) => {
      ...
      // =============================================================================
      // Self-Healing Module Message Handlers
      // =============================================================================
      ...
      if (request.type === 'FILL_FROM_PROFILE') {
        ...
        Promise.resolve().then(async () => {
          ...
          // Extract form fields for mapping
          const formFields = [];
